  ...
  setHandler() {
    ...
    sock?.ev.on('messages.upsert', async (m) => {
      ...
      m.messages.map(async (msg) => {
        ...
        } else if (config.webhookBase64) {
          let base64Type = ''
          let base64 = ''

          switch (messageType) {
            case 'imageMessage':
              base64 = await downloadMessage(msg.message.imageMessage, 'image')
              base64Type = msg.message.imageMessage.mimetype
              break
            case 'videoMessage':
              base64 = await downloadMessage(msg.message.videoMessage, 'video')
              base64Type = msg.message.videoMessage.mimetype
              break
            case 'audioMessage':
              base64Type = msg.message.audioMessage.mimetype
              base64 = await downloadMessage(msg.message.audioMessage, 'audio')
              break
            default:
              webhookData['msgContent'] = ''
              break
          }
          webhookData['msgContent'] = `data:${base64Type};base64,${base64}`
        }
        if (
          ['all', 'messages', 'messages.upsert'].some((event_wpp) =>
            config.webhookAllowedEvents.includes(event_wpp)
          )
        ) {
